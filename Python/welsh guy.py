...
class sumScreen(tk.Tk):
    def __init__(self, title, *args, **kwargs):
        ...
        self.title(title)
        self.background1 = tk.PhotoImage(file = "11.jpg")
        ...
        #self.testButton = tk.Button(self, text="calculate", font=self.customFont,fg="#EEE8AA", bg = "black", \
                                     #command=lambda: self.calculate(self.variable.get(),))###
        #self.testButton.grid(row=5, column=1, columnspan=1, sticky='NSEW')
...
'''go = True
while go:
    print("bin to den (1), den to bin(2), hex to bin(3), ")
    select = input("bin to hex(4), den to hex(5) or hex to den (6)? ")
    number = input("number: ")
    

    if select == "1":
        print(denarise(number))
        print("")

    elif select == "2":
        print (binarise(number))
        print("")
        
    elif select == "3":
        print(hexabinarise(number))
        print("")
        
    elif select == "4":
        print(binhexify(number))
        print("")
        
    elif select == "5":
        bina = unspace(unspace(binarise(number)))
        ans = binhexify(bina)
        print (ans)
        print("")
        
    elif select == "6":
        bina = unspace(unspace(hexabinarise(number)))
        ans = denarise(bina)
        print(ans)
        print("")
    elif select == "a":
        go = True
        while go:
            steve = random.randint(4951760157141521099596496896,79228162514264337593543950336)
            print(binarise(steve))        
